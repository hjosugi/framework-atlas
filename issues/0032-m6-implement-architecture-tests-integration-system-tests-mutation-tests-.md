...
## Acceptance
- [x] unit/integration/system/architecture/mutation testを混同しない。
- [x] polling testにtimeout/observable effect必須。
- [x] real dependencyとuncontrolled dependencyのtest-double方針を比較。
- [x] architecture conventionをcompile/gate/reviewのどこで保つか記録。
- [x] code coverageをtest adequacyと同一視しない。
- [x] local/CI同一build logicの利点とtooling costを記録。
